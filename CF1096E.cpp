...
inline int solve(int n, int m, int lim) {
    ...
    if (n < 0) return 0;
    int ans = 0;
    for (int i = 0; i <= m; ++i) ans = (ans + ((i & 1)? 998244352 : 1) * C(m, i) % mod * C(n - lim * i + m - 1, m - 1)) % mod;
    return ans;
}
...
signed main() {
    ...
    for (int x = r; x <= s; ++x)
        for (int i = 1; i <= p; ++i) {
            ...
            ans = (ans + C(p - 1, i - 1) * solve(s - i * x, p - i, x) % mod * inv[i]) % mod;
        }
    cout << ans * pow(C(s - r + p - 1, p - 1), mod - 2) % mod << endl;
    return 0;
}