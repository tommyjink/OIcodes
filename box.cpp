...
string solveParallelBeam() {
    ...
    for (int i = 0; i < t; ++i) {
        for (int r = 0; r < 7; ++r) {
            for (int c_idx = 0; c_idx < 7; ++c_idx) {
                ...
                else if (i == 5) { if (!(r >= 1 && r <= 5 && c_idx >= 1 && c_idx <= 5)) is_dead[i][r][c_idx] = true; }
    ...
    cout << ">> 检测到核心数: " << num_threads << "，启动 6 图并行集束搜索..." << endl;
    ...
    // 【集束宽度扩大】应对 6 图的庞大状态空间
    constexpr size_t BEAM_WIDTH = 500000; 
...
int main() {
    ...
    for (int i = 0; i < 7; ++i) {
        for (int j = 0; j < 7; ++j) {
            c[0][i][j] = map1[i][j]; c[1][i][j] = map2[i][j]; c[2][i][j] = map3[i][j]; 
            c[3][i][j] = map4[i][j]; c[4][i][j] = map5[i][j]; c[5][i][j] = map6[i][j];
        }
    }
    ...
    return 0;
}//clang++ box.cpp -o final_box -std=c++20 -O3 -mcpu=apple-m4 -pthread -Wall && ./final_box