...
void dfs1(int x,int fa,int d)
{
	...
	// printf("x:%lld,mson:%lld,cson:%lld,son:%lld\n",x,mson[x],cson[x],son[x]);
}
void dfs2(int x,int fa,int fal)
{
	maxl[x]=max(hei[x],fal);
	public_maxl=max(public_maxl,maxl[x]);
	// printf("x:%lld,fal:%lld,maxl:%lld\n",x,fal,maxl[x]);
	for(int i=head[x];i;i=e[i].next)
	{
		int v=e[i].to;
		if(v==fa)continue;
...
void solve()
{
	...
	for(int i=1;i<=n;i++)
	{
		...
		// printf("i:%lld,maxl:%lld,p:%lld,p_m:%lld\n",i,maxl[i],p,public_maxl);
